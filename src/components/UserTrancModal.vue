<template>
    <b-modal v-model="isVisible">
      ...
      <div>
        <h5 class="mb-3">Name: {{ selectedItem.user }}</h5>
        ...
        <b-row class="mt-3">
          ...
          <b-col md="6">
            ...
            <BForm @submit.stop.prevent>
              <BFormInput id="out-date" v-model="outDate" :state="outDateValid" placeholder="Fill out date" required disabled />
          ...
          <b-col md="6" class="mb-3">
            ...
            <BForm @submit.stop.prevent>
              <BFormInput id="entry-date" v-model="entryDate" :state="entryDateValid" placeholder="Fill entry date" required disabled />
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const selectedStatus = ref(props.selectedItem?.status);
      ...
      watch(
        ...
        (newItem) => {
          if (newItem) {
            outDate.value = formatDate(newItem.out) || '';
            entryDate.value = formatDate(newItem.entry) || '';
            selectedStatus.value = newItem.status || 'pending';
            console.log('Status updated:', selectedStatus.value);
          }
        },
        { immediate: true }
      );
      ...
      const handleAction = async () => {
        loading.value = true;
        console.log('Updating transaction with status:', selectedStatus.value);
